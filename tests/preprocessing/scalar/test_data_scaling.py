...
from src.preprocessing.scaler.data_scaling import (
    min_max_scale_combined,
...
class TestScalingMethods(unittest.TestCase):
    ...
    def test_min_max_scale_combined(self):
        ...
        combined_min = min(
            combined_train["A"].min(), combined_train["B"].min(), combined_test["A"].min(), combined_test["B"].min()
        )
        combined_max = max(
            combined_train["A"].max(), combined_train["B"].max(), combined_test["A"].max(), combined_test["B"].max()
        )
