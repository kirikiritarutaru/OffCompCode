...
def vis_simple_reg():
    ...
    X_plt = pd.DataFrame(
        np.arange(X.min(), X.max(), 0.01)[:, np.newaxis], columns=X_train.columns
    )
...
def vis_decision_tree():
    ...
    plt.xticks(
        range(len(indices)), X.columns[indices], rotation=90
    )  # X軸に特徴量の名前を追加
    plt.show()  # プロットを表示
...
def vis_depth_one_decision_tree():
    ...
    dot_data = tree.export_graphviz(
        model, out_file=None, rounded=True, feature_names=X_train.columns, filled=True
    )
    source = Source(dot_data, format="png")
    png_bytes = source.pipe(format="png")
    sio = io.BytesIO(png_bytes)
    ...
    for i in range(1, len(X_train)):
        ...
        SSE = np.sum(y_error_left * y_error_left) + np.sum(
            y_error_right * y_error_right
        )
        print("SSE:", SSE)
        loss.append(SSE)
        # MSEの計算
...
def lightgbm_proc():
    ...
    explainer = shap.TreeExplainer(
        model=model, feature_pertubation="tree_path_dependent"
    )
    # SHAP値の計算
...
def logistic_reg_proc():
    ...
    X = pd.concat(
        [
            X,
            pd.get_dummies(
                X["marital-status"], prefix="marital-status", drop_first=True
            ),
        ],
        axis=1,
    )
    X = pd.concat(
        [X, pd.get_dummies(X["occupation"], prefix="occupation", drop_first=True)],
        axis=1,
    )
    X = pd.concat(
    ...
    X = pd.concat(
        [X, pd.get_dummies(X["race"], prefix="race", drop_first=True)], axis=1
    )
    X = pd.concat(
